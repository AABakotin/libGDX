...
public class Hero {
    ...
    public void update(float dt) {
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.S)) {
            position.x -= MathUtils.cosDeg(angle) * 120f * dt;
            position.y -= MathUtils.sinDeg(angle) * 120f * dt;
            lastDisplacement.set(MathUtils.cosDeg(angle) * 120f * dt, MathUtils.sinDeg(angle) * 120f * dt);
